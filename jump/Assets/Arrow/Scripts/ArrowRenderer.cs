...
public class ArrowRenderer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Vector3 bodyWorldPosition = bodyTransform.position;
            if (playerScript != null)
            {
                directionangle = playerScript.directionangle;
                estimatetime = playerScript.estimatetime ;
                player_speed = playerScript.total_speed;
            }
            else
            {
                Debug.Log("Player script not assigned!");
            }
            Vector3 endposition = bodyWorldPosition + new Vector3((float)(estimatetime * System.Math.Sin(directionangle)), 0, (float)(estimatetime*System.Math.Cos(directionangle))) * player_speed;
            SetPositions(bodyWorldPosition, endposition);
        }
        else
        {
            Debug.Log("Body component not found!");
        }
        UpdateSegments();
    }
